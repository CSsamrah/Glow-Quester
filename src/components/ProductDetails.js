...
export default function ProductDetails() {
    ...
    const deleteProduct = async (product_id) => {
        try {
            const response = await fetch(`/product/${product_id}`, {
                method: 'DELETE',
            });
            if (response.ok) {
    ...
    const editProductQuantity = async (product_id, quantity) => {
        try {
            const response = await fetch(`/product/${product_id}/quantity`, {
                method: 'PUT',
                headers: {
    ...
    const editProductPrice = async (product_id, price) => {
        try {
            const response = await fetch(`/product/${product_id}/price`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
